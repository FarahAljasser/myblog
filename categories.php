            ...
            <div class="row" style="text-align: right;">
              ...
              <div class="col-md-10" class="main-area">
                <div class="add-category">
                    ...
                    if(isset($cAdd)){
        if(empty($cName)){
            echo "<div class='alert alert-danger'>" .'حقل الفئة فارغ'. "</div>";
            
        }elseif($cName>100){
            echo "<div class='alert alert-danger'>" .'اسم الفئة كبير جدا !'. "</div>";
            
        }else{
          $query = "INSERT INTO categories(categoryName)VALUES('$cName')";
          mysqli_query($conn,$query);
          echo "<div class='alert alert-success'>" .'تمت اضافة الفئة بنجاح '. "</div>";
        }
    }
                    ?>
                  <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
                    <div class="form-group">
                    <label for="category"> فئة جديدة  </label>
                    <input type="text" name="category" class="form-control">
                    </div>
                    <button class="btn-custom form-cotrol" name="add"> اضافة </button>
                  </form>
                </div>
                                ...
                                while($row = mysqli_fetch_assoc($res )){
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="categories.php?id=<?php echo $row['id']; ?>">
                                          <button class="custom-btn">حذف الفئة </button></a></td>
                                    </tr>
